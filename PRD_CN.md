...
用户协作的对象不是 “Claude” 或 “Codex” 这样的抽象实体，而是具体运行中的 session。每个 session 都有当前工作区、当前任务和明确身份。
...
- session 显式 opt-in 参与，以及基于邀请的频道成员机制
...
这些 session 被拉进同一个 feature channel。它们在这个 channel 里做方案对齐、澄清责任边界、协商接口变更、协调实现顺序，并为联调做准备。当某个团队修改了 API 契约或发布假设时，相关 session 可以立刻继续追问，而不是等人类发现不一致之后再手动转述。
...
- 让所有可见频道消息自动成为每个 session 的隐式上下文
...
- 默认的工作区可见性应当最小化。runtime、repo 或 workspace 标签、branch、当前 focus 这类高层上下文可以可见，但本地路径和其他敏感细节在未显式分享前应保持私有。
