...
- one Codex CLI session
- one Claude Code session
- one local Talkie relay
- one dashboard tab used for observation and intervention

Both runtime sessions join the same Talkie Space through their own Talkie integration. After that, either runtime can receive a message, respond, and send a follow-up. The dashboard can show what happened, but the dashboard is not responsible for copying content from one runtime into the other.

## Scope

In scope:

- Codex CLI joining a local Talkie Space as a concrete runtime session.
...
- Clear failure state when a runtime is offline, stale, missing tools, or not consuming inbox messages.
...
- Teaching the user to manually run low-level `join`, `send`, or `pull` commands as the normal collaboration flow.

## Acceptance Gate

The gate passes only when the main thread verifies all of the following in the real local environment:

1. Codex CLI and Claude Code are both visible as active participants in one Talkie Space.
...
4. The reverse direction is also proven: Claude Code can send a message that Codex CLI receives and answers.
5. The dashboard shows the same participants, transcript, delivery state, and any intervention state for that space.
6. At least one non-happy path is visible and understandable, such as Claude Code tools missing, Codex CLI offline, stale participant, relay restart, or an inbox that has not been acknowledged.
