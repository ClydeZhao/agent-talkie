...
Use the same session client and WebSocket path as any other consumer: handshake, `session.register`, then validated envelopes on the open socket. Do not fork or bypass relay wire semantics at the adapter layer.
...
- Overload (queue overflow) is reported on **stderr** only; it does not surface as an error on the relay protocol stream.
