...
1. **Wire casing convention (camelCase vs snake_case)?** — RESOLVED: Use snake_case per D-01 in CONTEXT.md. Consistent with Zod property names and JSON convention.
2. **Where does schema rejection get surfaced for pure NATS subscribers (no HTTP)?** — RESOLVED: Phase 1 provides a reference validator function + documented error object. Global negative ack deferred to relay design (Phase 3).
3. **JetStream duplicate_window default vs collaboration SLA?** — RESOLVED: Claude discretion per CONTEXT.md. Tune in Compose config; document relationship to application-level dedup.
