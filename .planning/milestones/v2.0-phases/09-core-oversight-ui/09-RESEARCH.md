...
- **名册 OVER-01 D-04–D-07：** 左栏紧凑卡片列表；`displayName`、runtime badge、workspace、行内 chips；人类/机器人图标区分，`isHuman`；orchestrator 图标叠加；**初始与周期性完整名册来自 HTTP `space-summary` 快照 + ~10s 轮询**；**`metadata.patch` 经 `onEnvelope` 在轮询间隔内增量更新**（与 09-CONTEXT D-06/D-23 一致；不依赖 `space.joined`/`space.left` 广播）；`<talkie-roster>` / `<talkie-roster-entry>`，roster 不用 Shoelace。
...
- **架构 D-22–D-23：** Bridge → 集中 store → 组件；store 含 roster Map、transcript 有序列表、errors、space 元数据；**roster 由 HTTP 快照初始化 + 轮询刷新，transcript 由 catch-up + live `onEnvelope` 构建**（与 09-CONTEXT D-23 一致）。
...
- Architecture：**HIGH** — 代码路径已核对；**初始 roster** 已决议为 HTTP 快照 + 轮询（见 Open Questions (RESOLVED)）  
