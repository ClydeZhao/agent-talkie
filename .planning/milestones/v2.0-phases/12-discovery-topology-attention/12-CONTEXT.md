...
- `packages/dashboard/src/store/dashboard-store.ts` — `DashboardStore`: `transcriptLines` (Array), `roster` (Map<sessionId, RosterRow>), `RosterRow.progress`, `RosterRow.blockedReason`. Search index builds from transcriptLines; attention lane filters from roster.
